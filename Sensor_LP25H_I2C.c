...
// Function for I2C communication protocol
void i2c_com()
{
	// Open i2c comms
	int i2c_fd = 0;
	...
	// ID check of sensor. The expected value of the byte of data from the who_am_I register should be 10111101 (documentation, or 0xBD in hex).
...
// Function for SPI communication protocol
void spi_com()
{
	...
	// Need to find what is the command to take measurement.
	// Send command one by one in between CS HIGH/LOW.

	uint8_t tx_buffer[6] = {0x20, 0x00, 0x20, 0x84, 0x21, 0x01}; // Currently NOT WORKING
	uint8_t rx_buffer[2];
